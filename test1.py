...
def Etc_Read_Reg(address, length):
    ...
    Addr.LANWord = address | READ_MASK

    print("Addr.LANWord:", hex(Addr.LANWord))

    xfrbuf[0] = Addr.LANByte[1]  # Address high byte
    xfrbuf[1] = Addr.LANByte[0]  # Address low byte
    ...
    for i in range(length + 1):
        xfrbuf[i + 2] = DUMMY_BYTE  # Fill dummy bytes after address bytes
